...
struct SignatureCipherAlgorithm
{
    ...
    alias InjectDescrambleFunction = string function(string, string);

    InjectDescrambleFunction[] challengeCallbacks = [
        (javascript, signatureCipher) {
            auto match = javascript.matchFirst(ctRegex!(i`=($(JS_VAR))\((\d,)*?decodeURIComponent\($(JS_VAR)\.s\)\)`.text));
            if(match.empty)
            {
                return "";
            }
            string challengeName = match[1];
            string optionalFirstArgument = match[2].strip(",");
            if(optionalFirstArgument == "")
            {
                return javascript.replace("})(_yt_player);", "descramble=" ~ challengeName ~ "})(_yt_player);") ~ "var descrambled = encodeURIComponent(descramble(decodeURIComponent('" ~ signatureCipher ~ "')));";
            }
            return javascript.replace("})(_yt_player);", "descramble=" ~ challengeName ~ "})(_yt_player);") ~ "var descrambled = encodeURIComponent(descramble(" ~ optionalFirstArgument ~ ", decodeURIComponent('" ~ signatureCipher ~ "')));";
        },


        (javascript, signatureCipher) {
            auto match = javascript.matchFirst(ctRegex!(i`(?:($(JS_VAR))\($(JS_VAR)\.url,$(JS_VAR)\.sp,$(JS_VAR)\.s\))`.text));
            if(match.empty)
            {
                return "";
            }
            ...
            return javascript.replace("})(_yt_player);", "descramble=" ~ challengeName ~ "})(_yt_player);") ~ "var result = descramble('', 's', '" ~ signatureCipher ~ "'); var descrambled = Object.entries(result).filter(function(entry) { return entry[1] instanceof Object && entry[1].s !== undefined; })[0][1].s;";
        }
    ];
    ...
    //change this function to include the optional arguments
    //eg. ["Nta"] or ["Fp", "3")] or ["V0", "b.url", "b.sp", "b.s"]
    string handleChallenge(string modifiedJavascript, string signatureCipher)
    {
        foreach(callback; challengeCallbacks)
        {
            string result = callback(modifiedJavascript, signatureCipher);
            if(result != "")
            {
                return result;
            }
        }
        throw new Exception("Failed to find signatureCipher challenge name");
    }
...
unittest
{
    ...
    auto algorithm = SignatureCipherAlgorithm("tests/base.min.js".readText(), new StdoutLogger());
...
unittest
{
    ...
    scope(success) writeln("OK\n".formatSuccess());
    auto algorithm = SignatureCipherAlgorithm("tests/5b77d519.js".readText(), new StdoutLogger());
...
unittest
{
    ...
    scope(success) writeln("OK\n".formatSuccess());
    auto algorithm = SignatureCipherAlgorithm("tests/643afba4.js".readText(), new StdoutLogger());
...
unittest
{
    ...
    scope(success) writeln("OK\n".formatSuccess());
    auto algorithm = SignatureCipherAlgorithm("tests/6450230e.js".readText(), new StdoutLogger());
...
unittest
{
    writeln("When video is VEVO song, should correctly decrypt video signature".formatTitle());
    scope(success) writeln("OK\n".formatSuccess());
    auto algorithm = SignatureCipherAlgorithm("tests/29a37ef6.js".readText(), new StdoutLogger());
...
unittest
{
    ...
    scope(success) writeln("OK\n".formatSuccess());
    auto algorithm = SignatureCipherAlgorithm("tests/87644c66.js".readText(), new StdoutLogger());
...
unittest
{
    ...
    scope(success) writeln("OK\n".formatSuccess());
    auto algorithm = SignatureCipherAlgorithm("tests/6c5cb4f4.js".readText(), new StdoutLogger());
